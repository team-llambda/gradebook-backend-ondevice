...
;(function(EDUPoint) {
	...
	;(function(WebServiceFunction) {
		...
		(WebServiceFunction =
			EDUPoint.WebServiceFunction || (EDUPoint.WebServiceFunction = {}))
	)
	function WebServiceFunctionParameter(serviceFunction) {
		switch (serviceFunction) {
			case WebServiceFunction.Gradebook:
				return '<Parms><ChildIntID>0</ChildIntID></Parms>'
			case WebServiceFunction.ChildList:
				return ''
		}
	}
	...
	class PXPWebServices {
		...
		processWebRequest(functionToRun) {
			...
			return new Promise((resolve, reject) => {
				this.post(fullURL, requestParameters).then(
					document => {
						const body = document.getElementsByTagName('string')[0]
		...
		getGradebook() {
			return new Promise((resolve, reject) => {
				this.processWebRequest(EDUPoint.WebServiceFunction.Gradebook)
					.then(xml => {
		...
		getChildList() {
			return new Promise((resolve, reject) => {
				this.processWebRequest(EDUPoint.WebServiceFunction.ChildList)
					.then(xml => {
...
function dateFromAmericanShortFormat(input) {
	...
	const dayNumber = +input.substring(
...
;(function(EDUPoint) {
	class Assignment {
		...
		static initializeFromElement(data) {
			...
			assignment.dueDate = dateFromAmericanShortFormat(
				data.getAttribute('DueDate')
			)
			return assignment
		}
		/**
...
function parseScore(scoreString) {
	...
	const assigned = +scoreString.substring(
		delimiterIndex + 1,
		scoreString.length - 1
	) // plus one for the trailing space.
...
;(function(EDUPoint) {
	class AssignmentGradeCalc {
		...
		static initializeFromElement(data) {
			...
			const weightValueAsDecimal =
				+weightValueAsPercentage.substring(
					0,
					weightValueAsPercentage.indexOf('%')
				) / 100
...
;(function(EDUPoint) {
	class Child {
		constructor(data) {
			...
			this.childName = data.getElementsByTagName(
				'ChildName'
			)[0].childNodes[0].nodeValue
			this.organizationName = data.getElementsByTagName(
				'OrganizationName'
			)[0].childNodes[0].nodeValue
...
;(function(EDUPoint) {
	class Gradebook {
		constructor(data) {
			...
			const allReportingPeriods = [
				...data.getElementsByTagName('ReportingPeriods')
			]
...
;(function(EDUPoint) {
	class Mark {
		constructor(data) {
			...
			this.assignments = []
			const gradeCalculationChildren = [
				...data.getElementsByTagName('AssignmentGradeCalc')
			]
			gradeCalculationChildren.forEach(element => {
				const gradeCalc = EDUPoint.AssignmentGradeCalc.initializeFromElement(
					element
				)
				if (gradeCalc.type != 'TOTAL') {
					this.gradeCalculation.push(
						EDUPoint.AssignmentGradeCalc.initializeFromElement(element)
					)
				}
			})
			...
			assignmentChildren.forEach(element => {
				this.assignments.push(
					EDUPoint.Assignment.initializeFromElement(element)
				)
			})
		...
		get calculateScore() {
			...
			const gradedAssignments = this.assignments.filter(
				value => value.actualScore != null
			)
			// If there are no grade calculation types, then just count it as is.
			if (this.gradeCalculation.length <= 0) {
				const totalActualScore = gradedAssignments.reduce(
					(accum, current) => (accum += current.actualScore),
					0
				)
				const totalAssignedScore = gradedAssignments.reduce(
					(accum, current) => (accum += current.assignedScore),
					0
				)
			...
			gradedAssignments.forEach(assignment => {
				calculations.forEach((markScore, index) => {
					if (
						assignment.type.length <= 0 ||
						markScore.type == assignment.type
					) {
...
;(function(EDUPoint) {
	class ReportingPeriod {
		constructor(data) {
			...
			this.startDate = dateFromAmericanShortFormat(
				data.getAttribute('StartDate')
			)
